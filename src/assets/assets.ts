const ASSETS = {
	logo: new URL("./logo.png", import.meta.url).href,
	alfred: new URL("./alfred.png", import.meta.url).href,
	ccm: new URL("./ccm.png", import.meta.url).href,
	elso: new URL("./elso.png", import.meta.url).href,
	em: new URL("./em.png", import.meta.url).href,
	jhm: new URL("./jhm.png", import.meta.url).href,
	litfl: new URL("./litfl.jpg", import.meta.url).href,
	vecmos: new URL("./vecmos.png", import.meta.url).href,
}

export default ASSETS
